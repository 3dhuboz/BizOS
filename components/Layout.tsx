...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  const { user, logout, cart, settings, connectionError, getLabel, businessConfig } = useApp();
    ...
    <div className="min-h-screen flex flex-col pb-20 md:pb-0 relative overflow-x-hidden">
      ...
      <header className={`hidden md:flex items-center justify-between px-8 py-4 fixed top-0 left-0 right-0 z-50 glass transition-all duration-300 ${connectionError ? 'mt-8' : ''}`}>
        <Link to="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col">
            <h1 className="text-2xl font-display font-bold text-white tracking-wider uppercase leading-none group-hover:text-bbq-red transition-colors">{settings.businessName || 'BizOS'}</h1>
            <span className="text-xs text-bbq-gold font-bold tracking-[0.2em] uppercase">{getLabel('tagline')}</span>
          </div>
        </Link>
        ...
        <nav className="flex items-center gap-8 bg-black/40 px-8 py-3 rounded-full border border-white/5 backdrop-blur-md">
          ...
          {businessConfig.features.menuBrowsing && (
            <Link to="/menu" className={`font-bold text-sm tracking-widest uppercase transition ${isActive('/menu')}`}>{getLabel('items')}</Link>
          )}
          {businessConfig.features.catering && (
            <Link to="/diy" className={`font-bold text-sm tracking-widest uppercase transition ${isActive('/diy')}`}>{getLabel('packages')}</Link>
          )}
          {businessConfig.features.bookingPortal && (
            <Link to="/booking" className={`font-bold text-sm tracking-widest uppercase transition ${isActive('/booking')}`}>Book</Link>
          )}
          {businessConfig.features.gallery && (
            <Link to="/gallery" className={`font-bold text-sm tracking-widest uppercase transition ${isActive('/gallery')}`}>{getLabel('gallery')}</Link>
          )}
          {businessConfig.features.rewards && (
          ...
          ) : businessConfig.features.onlineOrdering ? (
             <Link to="/order" className={`font-bold text-sm tracking-widest uppercase transition ${isActive('/order')}`}>{getLabel('booking')}</Link>
          ) : null}
        </nav>
      ...
      <header className={`md:hidden flex items-center justify-between p-4 fixed top-0 w-full z-50 glass ${connectionError ? 'mt-8' : ''}`}>
         <Link to="/" className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-display font-bold text-white uppercase tracking-tight">{settings.businessName || 'BizOS'}</h1>
        </Link>
      ...
      <footer className="relative bg-bbq-charcoal border-t border-white/5 pt-16 pb-24 md:pb-8 mt-12 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10 grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div className="space-y-4">
             ...
             <h3 className="font-display font-bold text-2xl tracking-wide uppercase text-white">{settings.businessName || 'BizOS'}</h3>
          ...
          <div>
            ...
            <ul className="space-y-3 text-gray-400 text-sm font-medium">
              ...
              <li><Link to="/menu" className="hover:text-white hover:translate-x-1 transition-all inline-block">{getLabel('items')}</Link></li>
              <li><Link to="/rewards" className="hover:text-white hover:translate-x-1 transition-all inline-block">Rewards Club</Link></li>
              <li><Link to="/gallery" className="hover:text-white hover:translate-x-1 transition-all inline-block">{getLabel('gallery')}</Link></li>
            ...
            <ul className="space-y-4 text-gray-400 text-sm">
              ...
              {settings.emailSettings?.fromEmail && (
                <li className="flex items-center gap-3">
                  ...
                  <a href={`mailto:${settings.emailSettings.fromEmail}`} className="hover:text-white transition">{settings.emailSettings.fromEmail}</a>
                </li>
              )}
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              <a href="https://facebook.com/bizos" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-3 rounded-xl hover:bg-[#1877F2] transition text-white border border-white/10">
                <Facebook size={20} />
              </a>
              <a href="https://www.instagram.com/bizos/?hl=en" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-3 rounded-xl hover:bg-gradient-to-tr hover:from-yellow-500 hover:to-purple-600 transition text-white border border-white/10">
                <Instagram size={20} />
              </a>
            </div>
            <p className="text-xs text-gray-600 mt-8 font-mono">
              © {new Date().getFullYear()} {settings.businessName || 'BizOS'}.<br/>All rights reserved.
            </p>
          </div>
        </div>
      </footer>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full glass border-t border-white/10 flex justify-around py-4 pb-safe z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        ...
        {businessConfig.features.menuBrowsing && (
          <NavItem to="/menu" icon={UtensilsCrossed} label={getLabel('items')} />
        )}
        {businessConfig.features.gallery && (
        ...
        {(user?.role === 'ADMIN' || user?.role === 'DEV') ? (
          ...
          <div className="relative">
             <NavItem to="/order" icon={CalendarDays} label={getLabel('booking')} />
          </div>
        ) : businessConfig.features.bookingPortal ? (
          <NavItem to="/booking" icon={CalendarDays} label="Book" />
        ) : null}
        {user ? (
