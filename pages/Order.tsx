        ...
        try {
            await Promise.allSettled([
                fetch('/api/v1/email/order-notification', {
                    ...
                    body: JSON.stringify({ settings: settings.emailSettings, order: orderPayload, businessName: settings.businessName })
                }),
                fetch('/api/v1/sms/order-notification', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ settings: settings.smsSettings, order: orderPayload, businessName: settings.businessName })
                })
            ]);
        } catch (e) {
