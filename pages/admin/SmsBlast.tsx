      ...
      try {
        const res = await fetch('/api/v1/sms/blast', {
          ...
          body: JSON.stringify({
            ...
            message: message.replace(/\{name\}/g, customer.name).replace(/\{business\}/g, settings.businessName || 'BizOS'),
          }),
        });
        if (res.ok) sent++;
        else failed++;
      ...
      <div className="bg-gray-900/50 p-5 rounded-xl border border-gray-800 space-y-4">
        <div>
          ...
          <textarea
            ...
            placeholder={`Hey {name}! 🍖 Big news from ${settings.businessName || 'BizOS'}...`}
