...
const Login: React.FC = () => {
  ...
  const handleSocialLogin = (platform: 'Google' | 'Facebook') => {
    ...
    const strategy = platform === 'Google' ? 'oauth_google' : 'oauth_facebook';
    ...
    try {
        if (mode === 'ADMIN') {
            ...
            if (result.status === 'complete') {
              ...
              toast('Additional verification required.', 'info');
            }
        } else {
            // SIGNUP
            ...
            if (result.status === 'complete') {
              ...
              toast('Please check your email to verify your account.', 'info');
            }
        }
    } catch (error: any) {
        const msg = error?.errors?.[0]?.longMessage || error?.message || 'Authentication failed';
        toast(msg, 'error');
        console.error(error);
    } finally {
        setIsLoading(false);
