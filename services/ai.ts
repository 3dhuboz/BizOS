...
// Module-level key that AppContext sets from settings — works in incognito/new devices
let _runtimeKey = '';
export const setAiApiKey = (key: string) => { _runtimeKey = key; };
...
const SITE_URL = 'https://bizos.app';
const SITE_NAME = 'BizOS';
...
export const generateSocialPost = async (topic: string, platform: 'Facebook' | 'Instagram') => {
  try {
    ...
    const text = await chat([{
      role: 'user',
      content: `You are an expert social media manager for "BizOS", a mobile BBQ catering business.
...
export const generateMarketingImage = async (prompt: string): Promise<string | null> => {
  const key = getApiKey();
  if (!key) {
    console.error('OpenRouter Image: No API key configured');
    return null;
  }

  const fullPrompt = `Generate a delicious, professional food photography style image of BBQ food: ${prompt}. High quality, appetizing, cinematic lighting, no text or watermarks.`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn('OpenRouter image failed:', res.status, errText.slice(0, 200));
      return null;
    }

    const data = await res.json();
    // Response format: choices[0].message.images[0].image_url.url = "data:image/png;base64,..."
...
export const generateSocialRecommendations = async (stats: any) => {
  try {
    ...
    return await chat([{
      role: 'user',
      content: `You are a social media strategist for "BizOS".
...
export const askPitmasterAI = async (history: { role: 'user' | 'model'; text: string }[], newMessage: string) => {
  try {
    ...
    const messages: { role: 'system' | 'user' | 'assistant'; content: string }[] = [
      {
        role: 'system',
        content: `You are 'Pitmaster Jay', the owner and head pitmaster of "BizOS".
...
6. If asked something unrelated to BBQ, meat, or BizOS, politely steer the conversation back to food or say you're busy checking the fire.
